...
public:
  ...
  void AutonomousPeriodic() override
  {
     ...
     frc::SmartDashboard::PutNumber("Aceleração", accel);
      ...
      if (m_encoderLeft.GetDistance()>1410.0 || m_encoderRight.GetDistance()>1410.0)
     {
        lAccel = 0;
        rAccel = 0;
     }
      else if (m_encoderLeft.GetDistance()>820.0 || m_encoderRight.GetDistance()>820.0)
     {
        lAccel = -0.75;
        rAccel = -0.75;
     }
     else if (m_encoderLeft.GetDistance()>590.0 || m_encoderRight.GetDistance()>590.0)
     {
        lAccel = 0;
        rAccel = -0.75;
     }
